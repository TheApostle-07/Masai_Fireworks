...
// Import selected firework options from common.js
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let selectedFireworkType = "circular"; // Default type
    let selectedFireworkColor = "Random"; // Default color
    ...
    function setFireworkType(type) {
        selectedFireworkType = type;
        console.log(`Selected Firework Type: ${type}`);
        launchFirework(); // Launch a firework immediately after selecting type
    }
    
    function setFireworkColor(color) {
        selectedFireworkColor = color;
        console.log(`Selected Firework Color: ${color}`);
    }

    function launchFirework() {
        const x = canvas.width / 2;
        const y = canvas.height;
        const targetX = x; // Launch vertically to the center of the canvas
        const targetY = canvas.height / 4; // Target height for explosion
        
        const firework = new Firework(x, y, targetX, targetY, false);
        createExplosion(targetX, targetY, selectedFireworkType, selectedFireworkColor === "Random" ? `hsl(${Math.random() * 360}, 100%, 50%)` : selectedFireworkColor);
    }
    ...
    class Particle {
        ...
        draw() {
            ...
            ctx.shadowBlur = 100; // Adjust this value for the desired glow intensity
    
            // Set the fill color and draw the particle
        ...
        update() {
            this.x += this.speedX;
            this.y += this.speedY;
            this.alpha -= 0.02; // Fades out over time
        }
    }
    ...
    class ConfettiParticle {
        constructor(x, y, isSpecial = false) {
            this.x = x;
            this.y = y;
            this.color = `hsl(${Math.random() * 360}, 100%, 50%)`; // Vivid color
            this.size = Math.random() * 3 + 2;
            this.speedX = (Math.random() - 0.5) * 6;
            this.speedY = (Math.random() - 0.5) * 6;
            this.alpha = 1;
            this.isSpecial = isSpecial; // Special flag to apply extra effects
        }
        ...
        draw() {
            ...
            ctx.strokeStyle = this.isSpecial ? "rgba(255, 215, 0, 0.4)" : "rgba(0, 0, 0, 0.2)"; // Gold for special, black for regular
            ctx.lineWidth = this.isSpecial ? this.size : this.size / 2; // Thicker outline for special
            ctx.beginPath();
            ctx.arc(this.x, this.y, this.size + 2, 0, Math.PI * 2); // Larger radius for outline in special case
            ctx.stroke();
    
            // Draw main particle with optional glow for special cases
            ctx.fillStyle = this.color;
            ctx.shadowColor = this.isSpecial ? "rgba(255, 223, 0, 0.6)" : "transparent"; // Glow for special particles
            ctx.shadowBlur = this.isSpecial ? 10 : 0; // Glow intensity for special
            ctx.beginPath();
            ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2);
            ctx.fill();
        ...
        update() {
            this.x += this.speedX;
            this.y += this.speedY;
            this.alpha -= 0.02; // Gradual fade out
        }
    }

    function createExplosion(x, y, explosionType, color = `hsl(${Math.random() * 360}, 100%, 50%)`, isSpecial = false) {
        ...
        for (let i = 0; i < numParticles; i++) {
            ...
            switch (explosionType) {
                case "circular":  // Default circular explosion
                    const angle = Math.random() * Math.PI * 2;
                    speedX = Math.cos(angle) * (Math.random() * 4 + 1);
                    speedY = Math.sin(angle) * (Math.random() * 4 + 1);
                    break;
    
                case "starburst":  // For Chrysanthemum-like effect
                    const spikeAngle = (i % 6) * (Math.PI / 3);
                    speedX = Math.cos(spikeAngle) * (Math.random() * 3 + 2);
                    speedY = Math.sin(spikeAngle) * (Math.random() * 3 + 2);
                    break;
    
                case "ring":  // Ring Shell with particles in a circular formation
                    const ringAngle = (Math.PI * 2) * (i / numParticles);
                    speedX = Math.cos(ringAngle) * 2;
                    speedY = Math.sin(ringAngle) * 2;
                    break;
    
                case "randomSpread":  // Randomly spread particles for a burst effect
                    speedX = (Math.random() - 0.5) * 8;
                    speedY = (Math.random() - 0.5) * 8;
                    break;
    
                case "willow":  // Willow with slow falling particles
                    const willowAngle = Math.random() * Math.PI * 2;
                    speedX = Math.cos(willowAngle) * 2;
                    speedY = Math.sin(willowAngle) * 0.5;
                    break;
    
                case "palm":  // Few large particles for a palm effect
                    const palmAngle = Math.random() * Math.PI * 2;
                    speedX = Math.cos(palmAngle) * (Math.random() * 3 + 1.5);
                    speedY = Math.sin(palmAngle) * (Math.random() * 2 + 1);
                    break;
    
                case "dahlia":  // Dense, bright burst resembling a dahlia flower
                    const dahliaAngle = Math.random() * Math.PI * 2;
                    speedX = Math.cos(dahliaAngle) * (Math.random() * 2.5 + 1.5);
                    speedY = Math.sin(dahliaAngle) * (Math.random() * 2.5 + 1.5);
                    break;
    
                case "kamuro":  // Large, glittery spread like a Kamuro effect
                    const kamuroAngle = Math.random() * Math.PI * 2;
                    speedX = Math.cos(kamuroAngle) * (Math.random() * 3 + 2);
                    speedY = Math.sin(kamuroAngle) * (Math.random() * 3 + 2);
                    break;
    
                case "crown":  // Brocade Crown with outward burst and slower fall
                    const crownAngle = Math.random() * Math.PI * 2;
                    speedX = Math.cos(crownAngle) * (Math.random() * 2.5 + 2);
                    speedY = Math.sin(crownAngle) * (Math.random() * 2.5 + 2);
                    break;
    
                case "spider":  // Spider Shell with fast, sharp bursts
                    speedX = (Math.random() - 0.5) * 10;
                    speedY = (Math.random() - 0.5) * 10;
                    break;
    
                default:  // Default to a general random spread
                    speedX = (Math.random() - 0.5) * 6;
                    speedY = (Math.random() - 0.5) * 6;
            }
            const particleColor = color === "Multi-color" ? `hsl(${Math.random() * 360}, 100%, 50%)` : color;
